...
responses <- read_tsv('../raw_data/snapp_responses.txt')  # largest list of responses 
...
responses %>% 
  group_by( user_id, `user_time(ms)`, created_at, filename ) %>% 
  mutate( n = n() )  %>% 
  arrange(desc(n)) 
...
responses <- 
  responses %>% 
  rename( 'response' = answer , 'id' = user_id, response_time = `user_time(ms)` )
...
responses <- 
  ...
  arrange(id, photo_key, created_at) %>%  
  mutate( taxa_repeat = row_number()) %>% 
  mutate( taxa_repeat = ifelse( is.na(photo_key), NA, taxa_repeat)) %>% 
  distinct()

responses %>% group_by( id, filename, created_at) %>% mutate( n = n() ) %>% arrange( desc(n)) %>% view
...
response_data %>% 
  group_by( id, filename,  datetime2, filename, response, score ) %>% 
  mutate( n = n()) %>% 
  arrange( desc(n))
